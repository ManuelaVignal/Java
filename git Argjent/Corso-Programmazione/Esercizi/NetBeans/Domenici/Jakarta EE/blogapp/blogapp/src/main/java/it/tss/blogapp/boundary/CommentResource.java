...
public class CommentResource {
    ...
    public Comment find(@PathParam("id") Long id) {
        return store.find(id).orElseThrow(() -> new NotFoundException("Comment non trovato, id=" + id));
    }

    @DELETE
    @Path("{id}")
    public void delete(@PathParam("id") Long id) {
        Comment found = store.find(id).orElseThrow(() -> new NotFoundException("Comment non trovato, id=" + id));
